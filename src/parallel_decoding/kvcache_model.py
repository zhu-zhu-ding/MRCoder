...
class KVCacheModel():
    def __init__(self, model : torch.nn.Module, temperature : float = 1, top_k : int = 0, top_p : float = 0) -> None:
        ...
        self._prob_history_dtype = torch.float16

        self._temperature = temperature
        self._top_k = top_k
        self._top_p = top_p

    def _store_prob_history(self, logits: torch.Tensor) -> torch.Tensor:
        return logits.to(self._prob_history_dtype)


    def _forward_with_kvcache(self, input_ids : torch.Tensor, prompt_len, use_debug = True) -> torch.Tensor:
        if self._past_key_values is None:
            ...
            self._prob_history = self._store_prob_history(self._prob_history)

            # self._prob_history = outputs.logits[:, self._prompt_length:, :]
            # for i in range(self._prob_history.shape[-2]):
            #     self._prob_history[:, i, :] = norm_logits(self._prob_history[:, i, :], self._temperature, self._top_k, self._top_p)
            ...
            not_cached_q = self._store_prob_history(not_cached_q)
    ...
    def _generate_with_kvcache(self, prefix : torch.Tensor, 
        ...
        for _ in range(gamma):
            q = self._forward_with_kvcache(x, prompt_len, use_debug)
            # next_tok = sample(q)
    ...
    @torch.no_grad()
    def generate(self, input : torch.Tensor, gamma : int, prompt_len = None) -> torch.Tensor:
        if not prompt_len:
            prompt_len = input.shape[1]
        output = self._generate_with_kvcache(input, gamma,prompt_len)
        return output
